...
typedef struct auth_chain_b_local_data {
    auth_chain_local_data base_data;
    int data_size_list_len;
    int data_size_list2_len;
    int * data_size_list;
    int * data_size_list2;
}auth_chain_b_local_data;
...
int bisect_left(int * first, int * last, int value) {
    int * ptr, *curr;
    int count, step;
    count = last - first;
    curr = first;
...
unsigned int auth_chain_b_get_rand_len(int datalength, shift128plus_ctx* random, uint8_t* last_hash, auth_chain_b_local_data *l_data, uint16_t overhead) {
    if (datalength > 1440)
        return 0;
    shift128plus_init_from_bin_datalen(random, last_hash, 16, datalength);

    int pos = bisect_left(l_data->data_size_list, l_data->data_size_list + l_data->data_size_list_len, datalength + overhead);
    int final_pos = pos + shift128plus_next(random) % l_data->data_size_list_len;
    if (final_pos < l_data->data_size_list_len)
        return l_data->data_size_list[final_pos] - datalength - overhead;
    
    pos = bisect_left(l_data->data_size_list2, l_data->data_size_list2 + l_data->data_size_list2_len, datalength + overhead);
    final_pos = pos + shift128plus_next(random) % l_data->data_size_list2_len;
    if (final_pos < l_data->data_size_list2_len)
        return l_data->data_size_list2[final_pos] - datalength - overhead;

    if (final_pos < (pos + l_data->data_size_list2_len - 1))
        return 0;
...
unsigned int auth_chain_b_get_client_rand_len(auth_chain_local_data *local, int datalength, uint16_t overhead) {
    return auth_chain_b_get_rand_len(datalength, &local->random_client, local->last_client_hash, (auth_chain_b_local_data *)local, overhead);
}

unsigned int auth_chain_b_get_server_rand_len(auth_chain_local_data *local, int datalength, uint16_t overhead) {
    return auth_chain_b_get_rand_len(datalength, &local->random_server, local->last_server_hash, (auth_chain_b_local_data *)local, overhead);
}

int auth_chain_b_pack_data(char *data, int datalength, char *outdata, auth_chain_local_data *local, server_info *server) {
    unsigned int rand_len = auth_chain_b_get_client_rand_len(local, datalength, server->overhead);
    int out_size = (int)rand_len + datalength + 2;
    outdata[0] = (char)((uint8_t)datalength ^ local->last_client_hash[14]);
    outdata[1] = (char)((uint8_t)(datalength >> 8) ^ local->last_client_hash[15]);
...
int auth_chain_b_pack_auth_data(auth_chain_global_data *global, server_info *server, auth_chain_local_data *local, char *data, int datalength, char *outdata) {
    const int authhead_len = 4 + 8 + 4 + 16 + 4;
    const char* salt = local->salt;
    int out_size = authhead_len;
    ...
    out_size += auth_chain_b_pack_data(data, datalength, outdata + out_size, local, server);
...
static int __int_cmp(const void * a, const void * b) {
    return (*(int*)a - *(int*)b);
}

void auth_chain_b_init_data_size(obfs * self) {
    auth_chain_local_data *l_data = (auth_chain_local_data *) self->l_data;
    auth_chain_b_local_data *b_data = (auth_chain_b_local_data *) l_data;
    server_info *server = &self->server;

    shift128plus_ctx random;
    shift128plus_init_from_bin(&random, server->key, server->key_len);

    b_data->data_size_list_len = shift128plus_next(&random) % 8 + 4;
    b_data->data_size_list = (int *)(malloc(sizeof(int) * b_data->data_size_list_len));
    for(int i = 0; i < b_data->data_size_list_len; i++) 
        b_data->data_size_list[i] = (int)(shift128plus_next(&random) % 2340 % 2040 % 1440);
    qsort(b_data->data_size_list, b_data->data_size_list_len, sizeof(int), __int_cmp);
    
    b_data->data_size_list2_len = shift128plus_next(&random) % 16 + 8;
    b_data->data_size_list2 = (int *)(malloc(sizeof(int) * b_data->data_size_list2_len));
    for(int i = 0; i < b_data->data_size_list2_len; i++) 
        b_data->data_size_list2[i] = (int)(shift128plus_next(&random) % 2340 % 2040 % 1440);
    qsort(b_data->data_size_list2, b_data->data_size_list2_len, sizeof(int), __int_cmp);
}

obfs * auth_chain_b_new_obfs() {
    obfs * self = new_obfs();
    self->l_data = malloc(sizeof(auth_chain_b_local_data));
    auth_chain_local_data_init((auth_chain_local_data*)self->l_data);
...
void auth_chain_b_dispose(obfs *self) {
    ...
    auth_chain_b_local_data *b_data = (auth_chain_b_local_data *) local;
    if (b_data -> data_size_list != NULL) {
        free(b_data -> data_size_list);
        b_data -> data_size_list = NULL;
    }
    if (b_data -> data_size_list2 != NULL) {
        free(b_data -> data_size_list2);
        b_data -> data_size_list2 = NULL;
    }
    free(b_data);
    self->l_data = NULL;
    dispose_obfs(self);
...
int auth_chain_b_client_pre_encrypt(obfs *self, char **pplaindata, int datalength, size_t* capacity) {
    ...
    if (len > 0 && local->has_sent_header == 0) {
        ...
        if (head_size > datalength)
            head_size = datalength;
        pack_len = auth_chain_b_pack_auth_data((auth_chain_global_data *)self->server.g_data, &self->server, local, data, head_size, buffer);
        buffer += pack_len;
        data += head_size;
        len -= head_size;
        local->has_sent_header = 1;
    ...
    int unit_size = server->tcp_mss - server->overhead;
    while ( len > unit_size ) {
        pack_len = auth_chain_b_pack_data(data, unit_size, buffer, local, &self->server);
        buffer += pack_len;
        data += unit_size;
        len -= unit_size;
    }
    if (len > 0) {
        pack_len = auth_chain_b_pack_data(data, len, buffer, local, &self->server);
        buffer += pack_len;
    }
    len = (int)(buffer - out_buffer);
    if ((int)*capacity < len) {
...
int auth_chain_b_client_post_decrypt(obfs *self, char **pplaindata, int datalength, size_t* capacity) {
    ...
    while (local->recv_buffer_size > 4) {
        ...
        int data_len = (int)(((unsigned)(recv_buffer[1] ^ local->last_server_hash[15]) << 8) + (recv_buffer[0] ^ local->last_server_hash[14]));
        int rand_len = auth_chain_b_get_server_rand_len(local, data_len, server->overhead);
        int len = rand_len + data_len;
        if (len >= 4096) {
            local->recv_buffer_size = 0;
            error = 1;
...
void auth_chain_b_set_server_info(obfs *self, server_info *server) {
    server->overhead = 4;
    memmove(&self->server, server, sizeof(server_info));
    auth_chain_b_init_data_size(self);
}